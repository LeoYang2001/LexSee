...
const SignUpScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        ...
        cfmPasswordLabelTop.value = withTiming(cfmPasswordLabel ? -30 : 16, { duration: timeDur });
        cfmPasswordLabelLeft.value = withTiming(cfmPasswordLabel ? -16 : 0, { duration: timeDur });
    }, [emailLabel, passwordLabel, cfmPasswordLabel]);

    const handleSigningUp = () => {
        if (!email || !password || !cfmPassword) return alert('Fill in all the fields, please!');
    ...
    return (
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View className="flex-1 flex px-10 items-center py-10">
                <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1 justify-between w-full mb-20">
                    <Text style={styles.outlinedText} className="text-5xl font-bold mt-14">Sign Up</Text>
                    <View className="pt-10">
                    {/* Email Input */}
                    <View className="mt-10 flex-row items-center rounded-2xl border-2 relative">
                        <Animated.View style={{ top: emailLabelTop, left: emailLabelLeft }} className="absolute ml-4 z-0">
                            <Text style={{ fontSize: 14, color: emailLabel ? 'black' : 'grey' }} className="font-semibold">Email</Text>
                        </Animated.View>
                        <TextInput
                    ...
                    {/* Password Input */}
                    <View className="mt-10 flex-row items-center rounded-2xl border-2 relative">
                        <Animated.View style={{ top: passwordLabelTop, left: passwordLabelLeft }} className="absolute ml-4 z-0">
                            <Text style={{ fontSize: 14, color: passwordLabel ? 'black' : 'grey' }} className="font-semibold">Password</Text>
                        </Animated.View>
                        <TextInput
                    ...
                    <View className="mt-10 flex-row items-center rounded-2xl border-2 relative">
                        <Animated.View style={{ top: cfmPasswordLabelTop, left: cfmPasswordLabelLeft }} className="absolute ml-4 z-0">
                            <Text style={{ fontSize: 14, color: cfmPasswordLabel ? 'black' : 'grey' }} className="font-semibold">Confirm Password</Text>
                        </Animated.View>
                        <TextInput
                            onBlur={() => { if (!cfmPassword) setCfmPasswordLabel(false); }}
